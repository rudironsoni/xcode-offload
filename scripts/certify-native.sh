...
set -- "$bin" native certify --cert-root "$cert_root" --mode "$mode"

if [ -n "${XCODE_STORAGE_CERT_HOME:-}" ]; then
  set -- "$@" --home "$XCODE_STORAGE_CERT_HOME"
fi
...
case "$keep_artifacts:${XCODE_STORAGE_CERT_KEEP_ARTIFACTS:-0}" in
  1:*|*:1|*:true|*:TRUE|*:yes|*:YES)
    set -- "$@" --keep-artifacts
    ;;
esac

case "${XCODE_STORAGE_CERT_ALLOW_SYSTEM:-0}" in
  1|true|TRUE|yes|YES)
    set -- "$@" --allow-system
    ;;
esac

case "${XCODE_STORAGE_CERT_ALLOW_SIM_DELETE:-0}" in
  1|true|TRUE|yes|YES)
